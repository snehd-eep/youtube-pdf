...
export function ModeSelector({ mode, onModeChange, disabled }: ModeSelectorProps) {
  ...
  const [tcTier, setTcTier] = useState<"basic" | "detailed">(
    mode === "technical-course-pro" ? "detailed" : "basic"
  );
  ...
  const handleTcTierChange = (tier: "basic" | "detailed") => {
    setTcTier(tier);
    onModeChange(tier === "basic" ? "technical-course" : "technical-course-pro");
  };

  const sdSelected = mode === "system-design" || mode === "system-design-pro";
  const tcSelected = mode === "technical-course" || mode === "technical-course-pro";

  return (
    <div className="flex flex-col gap-3">
      {/* First row: Normal, System Design, Technical Course */}
      <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  {sdTier === "detailed"
                    ? "Deep analysis with multiple diagram types"
        ...
        >
          <button
            type="button"
            onClick={() => handleTcTierChange(tcTier)}
            disabled={disabled}
            className="w-full p-4 text-left"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  {tcTier === "detailed"
                    ? "Complete course with code examples"
                    : "Course summary with lessons"}
                </p>
              </div>
            </div>
          </button>
          ...
          <div className="px-4 pb-3 pt-0">
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => handleTcTierChange("basic")}
                disabled={disabled}
                className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-medium transition-all ${
                  tcTier === "basic"
              ...
              <button
                type="button"
                onClick={() => handleTcTierChange("detailed")}
                disabled={disabled}
                className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-medium transition-all ${
                  tcTier === "detailed"
                    ? "bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 ring-1 ring-amber-300 dark:ring-amber-700"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
              Full structured document with sections, definitions & Q&A
            </p>
          </div>
        </div>
      </button>
